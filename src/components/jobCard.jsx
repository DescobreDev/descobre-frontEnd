...
function PriorityBadge({ value }) {
  const cfg = PRIORITY_CONFIG[value] ?? { label: value, color: "#64748b", bg: "#f1f5f9", dot: "#94a3b8" };
  return (
    <span style={{
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 16,
        padding: 18,
        border: `1px solid ${hovered ? "#e2e8f0" : "#eef1f6"}`,
        boxShadow: hovered ? "0 14px 32px rgba(15,23,42,0.10)" : "0 1px 2px rgba(15,23,42,0.04)",
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        <div style={{ minWidth: 0 }}>
          <h3 style={{
            margin: 0, fontSize: 15.5, fontWeight: 700, color: "#0f172a",
            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          }}>
            {job.title}
          </h3>
          <p style={{ margin: "4px 0 0", fontSize: 12.5, color: "#64748b", display: "flex", alignItems: "center", gap: 4 }}>
            <MapPin size={12} />
            {job.city ? `${job.city} · ${job.state}` : "Remoto"}
          </p>
        </div>

        {/* stopPropagation: clicar no status/menu não deve abrir o detalhe da vaga */}
      ...
      <div style={{
        ...
        <span style={{ fontSize: 12.5, fontWeight: 500, color: deadline?.isPast ? "#dc2626" : "#94a3b8" }}>
          {deadline
